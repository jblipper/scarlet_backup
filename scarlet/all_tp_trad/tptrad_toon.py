...
pdb.set_trace()
#multiScatToon(self,IrradStar,extinctCoef,scatCoef,asym = 0,u0 = 0.5773502691896258,refl = False,thermal = True,calcJ = False,intTopLay=0,method ='Hemispheric Mean',w = -1)
...
def tp_trad(T,do_scat=True,negli_flux=0):
    ...
    extinctCoef_long=atm.opac['extinctCoef'].copy()
    ...
    print(scatCoef)
    aa=np.ones(1)*atm.dz[-1]
    atm.dz=np.concatenate((atm.dz, aa), axis=0)
    print(np.shape(np.outer(atm.dz,np.ones(atm.nWave, dtype=atm.numerical_precision))))
    print(np.shape(extinctCoef))
    
    N=len(T)
    levels=T.copy()
    
    #toon=atm.multiScatToon(atm.IrradStarEff,extinctCoef,scatCoef,asym = 0,u0 = 0.5773502691896258,refl = True,thermal = True,calcJ = False,intTopLay=atm.calcEmissionSpectrum(extinctCoef_long),method ='Hemispheric Mean',w = -1)
    ...
    #----------------------------------------------------------------------------# 
    cum_trans=np.ones((N,N,len(atm.wave)),dtype=atm.numerical_precision)*-1
    def get_cum_trans(shallow_level,deep_level):
        if shallow_level>deep_level:
            ...
            out=np.e**(-dz[shallow_level]/l[shallow_level])
            cum_trans[shallow_level][deep_level]=out.copy()
            print("SJHSNJKNS")
            return out
        else:
            out=np.e**(-dz[deep_level]/l[deep_level])*get_cum_trans(shallow_level,deep_level-1)
            cum_trans[shallow_level][deep_level]=out.copy()
            print("SJHSNJKNS")
            return out
    ...
    #
    #flux_star=np.zeros((N,len(atm.wave)))
    flux00=get_flux(T,flux_star)#+toon[2]
    temp1=np.array([atm.wave[1]-atm.wave[0]])
    temp2=atm.wave[1:]-atm.wave[:atm.nWave-1]
    flux_up_top=atm.calcEmissionSpectrum(atm.extinctCoef)
    flux_down_top=atm.IrradStar*np.concatenate((temp1,temp2),axis=0) #EffIntensityPerHz
    flux0_top=np.array([flux_up_top-flux_down_top])
    #flux0_top=np.array([np.zeros(atm.nWave)])
    flux0=np.concatenate((flux0_top,toon[2]),axis=0)             
    pdb.set_trace()
    #------------------------------------------------------------------------#
    ...
    for level_i in range(N):
         ...
         atm.T=levels_ptb
         flux_ptb_0=atm.multiScatToon(atm.IrradStar,extinctCoef,scatCoef,refl = True,thermal = True)[2]
         flux_up_top=atm.calcEmissionSpectrum(atm.extinctCoef)
         flux0_top=np.array([flux_up_top-flux_down_top])
         flux_ptb=np.concatenate((flux0_top,flux_ptb_0),axis=0)
    ...
    #aa=np.zeros(1)
    #levels=levels+np.concatenate((delta_levels, aa), axis=0)
