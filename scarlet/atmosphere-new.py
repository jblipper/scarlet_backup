...
#         return dppm
    ...
    #%% Scattering
    #multiScatToon
    def multiScatToon(self,IrradStar,extinctCoef,scatCoef,temperature_profile,asym = 0,u0 = 0.5773502691896258, intTopLay=0,method_for_therm ='Hemispheric Mean', method_for_ref='Quadrature', hard_surface=False, surf_reflect=0, mid=False, ftau_cld=0, b_top_for_ref=0, ref=True, therm=True, delta_approx=True):
        print('Running Toon')
        ...
        if ref==True:
            ...
            flux_minus_all = np.zeros((nlev,self.nWave))
            flux_plus_all = np.zeros((nlev,self.nWave))
            if mid==True:
                flux_minus_mdpt_all = np.zeros((nlev,self.nWave))
                flux_plus_mdpt_all = np.zeros((nlev,self.nWave))        
            ...
            for g in range(len(ulist)):
                ...
                if method_for_ref == 'Eddington': #Eddington Method
                    ...
                    gam1  = (np.sqrt(3)*0.5)*(2. - w0*(1.+asym)) #Table 1 Toon et Al.
                    gam2  = (np.sqrt(3)*w0*0.5)*(1.-asym)        #Table 1 Toon et Al.
                    gam3  = 0.5*(1.-np.sqrt(3)*asym*u0) #Table 1 Toon et Al.
                ...
                flux_minus=np.zeros(shape=(nlev,self.nWave))
                flux_plus=np.zeros(shape=(nlev,self.nWave))
                    
                if mid==True:
                    flux_minus_midpt = np.zeros(shape=(nlev,self.nWave))
                    flux_plus_midpt = np.zeros(shape=(nlev,self.nWave))
