...
#            self.thermal=convolve(self.thermal,kernel)
    ...
    def calcAtmosphere(self,modelSetting,params,updateOnlyDppm=False,updateTp=True,disp2terminal=False,returnOpac=False,
        ...
        if modelSetting['ComposType']=='ChemKinetic' and (modelSetting['TempType']=='NonGray' or modelSetting['TempType']=='NonGrayConv'):
            nbIterations = 2
        else:
            nbIterations = 1
        ...
        if updateOnlyDppm is False:        
            ...
            if 'Toon' in modelSetting['thermalOnly']:
                extinctCoef = 0.5*(self.extinctCoef[:self.nLay-1,:]+self.extinctCoef[1:self.nLay,:])
                scatCoef = 0.5*(self.scatCoef[:self.nLay-1,:]+self.scatCoef[1:self.nLay,:])
                Fupw, Fdwn, Fnet = self.multiScatToon(0,extinctCoef,scatCoef,refl = False,thermal = True)
            ...
            if 'Toon' in modelSetting['albedoOnly']:
                ...
                self.albedoToon = Fupw[0,:] / IrradStar
...
#         return dppm
    ...
    def multiScatToon(self,IrradStar,extinctCoef,scatCoef,asym = 0,u0 = 0.5773502691896258,refl = False,thermal = True,calcJ = False,intTopLay=0,method ='Hemispheric Mean',w = -1):
        '''
        ...
        if w == -1:
            self.omega = scatCoef/extinctCoef
        else:
            self.omega = w*np.ones_like(extinctCoef, dtype=self.numerical_precision)
        
        self.dz   = -np.diff(self.z)        
        taun = extinctCoef * np.outer(self.dz,np.ones(self.nWave, dtype=self.numerical_precision))

        self.Fs = IrradStar
        self.u0 = u0
        [l,d] = np.shape(self.omega)
        
        tauc=np.vstack([np.zeros(len(self.wave), dtype=self.numerical_precision),np.zeros(len(self.wave), dtype=self.numerical_precision),np.cumsum(taun,axis=0)])
        tauc = np.delete(tauc,(l+1),axis=0)
        tauc[0,:] = np.nan
        
        taun=np.vstack([np.zeros(len(self.wave)),taun])
        taun[0,:] = np.nan
        
        self.omega = np.vstack([np.zeros(len(self.wave), dtype=self.numerical_precision),self.omega])
        self.omega[0,:] = np.nan
        
        if refl:
            GroundAlbedo = self.GroundAlbedo
        else:
            GroundAlbedo = 0
        ...
        #method = 'Quadrature' #added  
        
        # Build co-eff for various approaches
        if method == 'Eddington':
            self.g1 = (7-self.omega*(4-3*asym))/4
            self.g2 = -(1-self.omega*(4-3*asym))/4
            self.g3 = (2-3*asym*self.u0)/4
            u1 = 1/2
        elif method == 'Quadrature':
            self.g1 = 3**(1/2)*(2-self.omega*(1+asym))/2
            self.g2 = self.omega*3**(1/2)*(1-asym)/2
            self.g3 = (1-3**(1/2)*asym*self.u0)/2
            u1 = 1/3**(1/2)
        elif method == 'Hemispheric Mean':
            self.g1 = 2-self.omega*(1+asym)
            self.g2 = self.omega*(1-asym)
            self.g3 = (1-3**(1/2)*asym*self.u0)/2
            u1 = 1/2
        
        self.g4 = 1 - self.g3
        
        self.lmbda = (self.g1**2 - self.g2**2)**(1/2)
        self.gamma = self.g2/(self.g1 + self.lmbda)
        
        e1 = 1 + self.gamma*np.exp(-self.lmbda*taun)
        e2 = 1 - self.gamma*np.exp(-self.lmbda*taun)
        e3 = self.gamma + np.exp(-self.lmbda*taun)
        e4 = self.gamma - np.exp(-self.lmbda*taun)
        
        # Initialize tridiagonal matrix (TDM)
        A = np.zeros([2*l+1,d], dtype=self.numerical_precision)
        B = np.zeros([2*l+1,d], dtype=self.numerical_precision)
        D = np.zeros([2*l+1,d], dtype=self.numerical_precision)
        E = np.zeros([2*l+1,d], dtype=self.numerical_precision)
        ...
        for j in range(1,l):
            ...
            E[k] = e3[j,:]*(self.CP(tauc[j+1,:],0,j+1) - self.CP(tauc[j,:],taun[j,:],j)) - e1[j,:]*(self.CM(tauc[j+1,:],0,j+1)-self.CM(tauc[j,:],taun[j,:],j)) 
            #print j,k
            # check indicies
        ...
        for j in range(1,l):
            ...
            E[k] = e2[j+1,:]*(self.CP(tauc[j+1,:],0,j+1) - self.CP(tauc[j,:],taun[j,:],j)) - e4[j+1,:]*(self.CM(tauc[j+1,:],0,j+1)-self.CM(tauc[j,:],taun[j,:],j))
            #print j,k
            # check indicies
        ...
        D[k] = 0
        E[k] = GroundAlbedo*self.u0*np.exp(-tauc[l,:]/self.u0)*np.pi*self.Fs - self.CP(tauc[l,:],taun[l,:],l) + GroundAlbedo*self.CM(tauc[l,:],taun[l,:],l)
        ...
        solution = np.vstack([np.zeros(len(self.wave)),self.TDMAsolver(A,B,D,E)], dtype=self.numerical_precision)
        solution[0] = np.nan
    ...
    #        FP1 = solution[1,:]*(np.exp(-self.lmbda[1,:]*(taun[1,:]-tautest)) + self.gamma[1,:]*np.exp(-self.lmbda[1,:]*tautest))
    #        FP2 = solution[2,:]*(np.exp(-self.lmbda[1,:]*(taun[1,:]-tautest)) - self.gamma[1,:]*np.exp(-self.lmbda[1,:]*tautest))
    #        Fupw = FP1 + FP2 + self.CP(tauc[1,:],tautest,1)
            for i in range(1,l+1):
                k = 2*i-1
                FP1 = solution[k,:]*(np.exp(-self.lmbda[i,:]*(0)) + self.gamma[i,:]*np.exp(-self.lmbda[i,:]*taun[i,:]))
                FP2 = solution[k+1,:]*(np.exp(-self.lmbda[i,:]*(0)) - self.gamma[i,:]*np.exp(-self.lmbda[i,:]*taun[i,:]))
                FupwSOL[i-1,:] = FP1 + FP2 + self.CP(tauc[i,:],taun[i,:],i)
            
            # Downward Flux
            FdwnSOL = np.zeros([l,d], dtype=self.numerical_precision)
            DirSOL = np.zeros([l,d], dtype=self.numerical_precision)

            for i in range(1,l+1):
                k = 2*i-1
                FM1 = solution[k,:]*(self.gamma[i,:]*np.exp(-self.lmbda[i,:]*(0)) + np.exp(-self.lmbda[i,:]*taun[i,:]))
                FM2 = solution[k+1,:]*(self.gamma[i,:]*np.exp(-self.lmbda[i,:]*(0)) - np.exp(-self.lmbda[i,:]*taun[i,:]))
                DirSOL[i-1,:] = self.u0*np.pi*self.Fs*np.exp(-(tauc[i,:]+taun[i,:])/self.u0)
                FdwnSOL[i-1,:] = FM1 + FM2 + self.CM(tauc[i,:],taun[i,:],i) + DirSOL[i-1,:]
                
            # Net Flux
            FnetSOL = np.zeros([l,d], dtype=self.numerical_precision)
            for i in range(1,l+1):
                k = 2*i-1               
                FnetSOL[i-1,:] = solution[k,:]*(e1[i,:]-e3[i,:])+solution[k+1,:]*(e2[i,:]-e4[i,:])+ self.CP(tauc[i],taun[i],i) - self.CM(tauc[i],taun[i],i)        
        ...
        if thermal:    
            ...
            Tgrid = np.zeros([l+2], dtype=self.numerical_precision)
            Tgrid[0] = np.nan
            Tgrid[1] = self.T[0]
            for i in range(2,l+1):
                Tgrid[i] = (self.T[i-2] + self.T[i-1])/2
            Tgrid[-1] = self.T[-1]
                            
            B0 = np.zeros([l+1,d], dtype=self.numerical_precision) ;  B0[0,:] = np.nan
            B1 = np.zeros([l+1,d], dtype=self.numerical_precision) ;  B1[0,:] = np.nan
            for i in range (1,l+1):
                B0[i,:] = rad.PlanckFct(Tgrid[i],self.wave,'um','W/(m**2*um)','rad')
                B1[i,:] = (rad.PlanckFct(Tgrid[i+1],self.wave,'um','W/(m**2*um)','rad') - B0[i,:])/taun[i,:]
                            
            # Define Hemispheric Mean 2 Stream Source Function parameters
            paramG = np.zeros([l+1,d], dtype=self.numerical_precision)  ;   paramG[0,:] = np.nan
            paramK = np.zeros([l+1,d], dtype=self.numerical_precision)  ;   paramK[0,:] = np.nan
            paramH = np.zeros([l+1,d], dtype=self.numerical_precision)  ;   paramH[0,:] = np.nan
            paramJ = np.zeros([l+1,d], dtype=self.numerical_precision)  ;   paramJ[0,:] = np.nan
            a1 = np.zeros([l+1,d], dtype=self.numerical_precision)  ;   a1[0,:] = np.nan
            a2 = np.zeros([l+1,d], dtype=self.numerical_precision)  ;   a2[0,:] = np.nan
            o1 = np.zeros([l+1,d], dtype=self.numerical_precision)  ;   o1[0,:] = np.nan
            o2 = np.zeros([l+1,d], dtype=self.numerical_precision)  ;   o2[0,:] = np.nan
            
            for i in range(1,l+1):
                j = 2*i-1
    
                paramG[i,:] = (solution[j,:] + solution[j+1,:]) * (1/u1 - self.lmbda[i,:])
                paramK[i,:] = (solution[j,:] - solution[j+1,:]) * (1/u1 - self.lmbda[i,:])
                paramH[i,:] = (solution[j,:] - solution[j+1,:]) * self.gamma[i,:] * (self.lmbda[i,:] + 1/u1)
                paramJ[i,:] = (solution[j,:] + solution[j+1,:]) * self.gamma[i,:] * (self.lmbda[i,:] + 1/u1)
                a1[i,:] = (B0[i,:] + B1[i,:]*(1/(self.g1[i,:] + self.g2[i,:])-u1)) 
                o1[i,:] = (B0[i,:] - B1[i,:]*(1/(self.g1[i,:] + self.g2[i,:])-u1)) 
           
            a2 = B1
            o2 = B1
            
            # Intensity downwards (through bottom of layer)
            IdwnIR = np.zeros([l,d], dtype=self.numerical_precision)
            
            # Intensity at the bottom of the top layer
            IM1 = intTopLay*np.exp(-taun[1,:]/u0)
            IM2 = paramJ[1,:]/(self.lmbda[1,:]*u0+1) * (1-np.exp(-taun[1,:]*(self.lmbda[1,:]+1/u0)))
            IM3 = paramK[1,:]/(self.lmbda[1,:]*u0-1) * (np.exp(-taun[1,:]/u0)-np.exp(-taun[1,:]*self.lmbda[1,:]))
            ...
            for j in range(1,l):
                ...
                IM2 = paramJ[i,:]/(self.lmbda[i,:]*u0+1) * (1-np.exp(-taun[i,:]*(self.lmbda[i,:]+1/u0)))
                IM3 = paramK[i,:]/(self.lmbda[i,:]*u0-1) * (np.exp(-taun[i,:]/u0)-np.exp(-taun[i,:]*self.lmbda[i,:]))
...
#            IM2 = paramJ[k,:]/(self.lmbda[k,:]*u0+1) * (1-np.exp(-taun[k,:]*(self.lmbda[k,:]+1/u0)))
#            IM3 = paramK[k,:]/(self.lmbda[k,:]*u0-1) * (np.exp(-taun[k,:]/u0)-np.exp(-taun[k,:]*self.lmbda[k,:]))
#            IM4 = o1[k,:]*(1-np.exp(-taun[k,:]/u0))
#            IM5 = o2[k,:]*(u0*np.exp(-taun[k,:]/u0)+taun[k,:]-u0)
#            ISurf = IM1 + IM2 + IM3 + IM4 + IM5
            ISurf = IdwnIR[-1,:]
            
            IupwIR = np.zeros([l,d], dtype=self.numerical_precision)
            IP1 = ISurf*np.exp(-taun[k,:]/u0)
            IP2 = paramG[k,:]/(self.lmbda[k,:]*u0-1) * (np.exp(-taun[k,:]/u0)-np.exp(-taun[k,:]*self.lmbda[k,:]))
            IP3 = paramH[k,:]/(self.lmbda[k,:]*u0+1) * (1-np.exp(-taun[k,:]*(self.lmbda[k,:]+1/u0)))
        ...
        if calcJ:
            JSOL = np.zeros([l,d], dtype=self.numerical_precision)
            DirSOL = np.zeros([l,d], dtype=self.numerical_precision)
                
            for i in range(1,l+1):
                k = 2*i-1      
                DirSOL[i-1,:] = self.u0*np.pi*self.Fs*np.exp(-(tauc[i,:]+taun[i,:])/self.u0)
                JSOL[i-1,:] = (solution[k,:]*(e1[i,:]+e3[i,:])+solution[k+1,:]*(e2[i,:]+e4[i,:])+ self.CP(tauc[i],taun[i],i) - self.CM(tauc[i],taun[i],i) + DirSOL[i-1,:]/u0)/(4*np.pi*u1)   
            
            JIR = np.zeros([l,d], dtype=self.numerical_precision)
            JIR[0,:] = IupwIR[1,:] - IupwIR[0,:] + intTopLay - IdwnIR[0,:]
            
            for i in range (1,l-1):
                JIR[i,:] = IupwIR[i+1,:] - IupwIR[i,:] + IdwnIR[i-1,:] - IdwnIR[i,:]
            JIR[-1,:] = sigmaSB*self.params['Tint']**4/np.pi + IdwnIR[-2,:] - IupwIR[-1,:]
            ...
            Fupw = FupwSOL + FupwIR
            Fdwn = FdwnSOL + FdwnIR
            self.DirSOL = DirSOL #dbg
            self.Fupw = Fupw
            self.Fdwn = Fdwn
            
            Fnet = np.zeros([l,d], dtype=self.numerical_precision)
            Fnet[0,:] = Fupw[1,:] - Fupw[0,:] + intTopLay*np.pi - Fdwn[0,:] + DirSOL[0,:]
            for i in range (1,l-1):
                Fnet[i,:] = Fupw[i+1,:] - Fupw[i,:] + Fdwn[i-1,:] - Fdwn[i,:] #+ self.u0*np.pi*self.Fs*np.exp(-(tauc[i+1,:]+taun[i+1,:])/self.u0)
            Fnet[-1,:] = sigmaSB*self.params['Tint']**4 + Fdwn[-2,:] - Fupw[-1,:] #+ self.u0*np.pi*self.Fs*np.exp(-(tauc[-1,:]+taun[-1,:])/self.u0)
    ...
    def CP(self,tauc,tau,n):
        ...
        '''      
        C = self.omega[n,:]*np.pi*self.Fs*np.exp(-(tauc+tau)/self.u0)*((self.g1[n,:]-1/self.u0)*self.g3 + self.g4*self.g2[n,:]) / (self.lmbda[n,:]**2 - 1/self.u0**2)
        return C
    
    
    def CM(self,tauc,tau,n):
        '''
        ...
        '''      
        C = self.omega[n,:]*np.pi*self.Fs*np.exp(-(tauc+tau)/self.u0)*((self.g1[n,:]+1/self.u0)*self.g4 + self.g2[n,:]*self.g3) / (self.lmbda[n,:]**2 - 1/self.u0**2)
        return C
    ...
    def TDMAsolver(self,a,b,c,d):
        ...
        --------------------------
        TDMAsolver(a,b,c,d)
        Solves the tridiagonal matrix algorithm
        '''      
        
        n = np.size(d,axis=0)-1
        ...
        x = np.zeros(np.shape(d), dtype=self.numerical_precision)
        # back substitute.
        x[n,:] = d[n,:]
