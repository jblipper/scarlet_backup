...
def get_fluxes_toon(atm,IrradStar,extinctCoef,scatCoef,asym = 0,u0 = 0.5773502691896258, intTopLay=0,method_for_therm ='Hemispheric Mean', method_for_ref='Quadrature', hard_surface=False, surf_reflect=0, mid=False, ftau_cld=1, b_top_for_ref=0, ref=True, therm=True):
    ...
    if ref==True:
        ...
        #ulist=np.array([0.09853,0.30453,0.56202,0.80198,0.96019])
        #ulist=np.array([0.99513358,0.95250091,0.82712012,0.59734205,0.27934726])#/100
        #ulist=np.array([0.26261275,0.32418423,0.41258848,0.51755171,0.62674115])
        #ulist=np.array([0.00328635,0.00410312,0.00532038,0.00685343,0.00858905])
        #gaussweights=np.array([0.015747,0.073908,0.146386,0.167174,0.096781])
        
        #gaussweights=np.array([0.00329519,0.0178429,0.04543932,0.0791996,0.10604736,0.1125058,0.09111902,0.0445508])
        #ulist=np.array([0.99900341,0.98952432,0.95798307,0.8905868,0.77815859,0.61861849,0.41750116,0.18773307])
        
        gaussweights=np.array([0.015747,0.073908,0.146386,0.167174,0.096781])
        ulist=np.array([0.99513358,0.95250091,0.82712012,0.59734205,0.27934726])
        
        
        #ulist=np.array([0.8872983346,0.5,0.1127016654])
        #gaussweights=np.array([0.27777777,0.4444444,0.27777777])*0.5
        ...
        for g in range(len(ulist)):
            ...
            if method_for_ref == 'Eddington': #Eddington Method
                ...
                gam1  = (sqrt(3)*0.5)*(2. - w0*(1.+asym)) #Table 1 Toon et Al.
                gam2  = (sqrt(3)*w0*0.5)*(1.-asym)        #Table 1 Toon et Al.
                gam3  = 0.5*(1.-sqrt(3)*asym*u0) #Table 1 Toon et Al.
        ...
        if mid==False:
            ...
            reflected_component = flux_plus_all,flux_minus_all
        else:
...
Fupw, Fdwn=get_fluxes_toon(atm,atm.IrradStar,extinctCoef,scatCoef,therm=True,ref=True)
...
atm.totalFluxToon=get_fluxes_toon(atm,atm.IrradStar,extinctCoef,scatCoef,therm=False,ref=True)[0][0]
