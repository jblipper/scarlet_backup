...
def take_step_tp_trad(TList,in_levels,N,extinctCoef,scatCoef,doplot=False,firstplot=False):
    ...
    if conv:
        ...
        unstable = (np.where(dTdz > dTdzAdiabat+1e-10)[0])#[::-1] 
        print(unstable)
        print(f'new levels: {new_levels}')
        while len(unstable)>0:
            print(unstable[0])
            print(unstable[0]+1)
            new_levels[unstable[0]+1]=new_levels[unstable[0]]+dTdzAdiabat[unstable[0]]*atm.dz[unstable[0]]
            dT=np.diff(new_levels)
            dTdz = dT / atm.dz
            unstable = (np.where(dTdz > dTdzAdiabat+1e-10)[0])
            print(unstable)
            print(f'new levels: {new_levels}')
