...
def take_step_tp_trad(TList,in_levels,N,extinctCoef,scatCoef,conv_zones,doplot=False,firstplot=False):
    ...
    for level_i in range(N):
        ...
        levels_ptb[level_i]=in_levels[level_i]+deltaT*(1-conv_zones[level_i])
        dT=np.diff(levels_ptb)
        dTdz = dT / atm.dz
        ...
        inx_conv_zones=np.where(conv_zones==1)[0]
        for i in inx_conv_zones:
            if bool(np.isin(i+1,inx_conv_zones)):
                levels_ptb[i+1]=levels_ptb[i]+dTdzAdiabat[i]*atm.dz[i]
    ...
    new_levels=in_levels+delta_levels_reduced#*(1-conv_zones)
    ...
    if conv==True:
        ...
        new_conv_zones=deepcopy(slopeTooSteep)
        for i in slopeTooSteep:
            if not bool(np.isin(i+1,new_conv_zones)):
                new_conv_zones=np.append(new_conv_zones,i+1)
        for i in new_conv_zones:
            conv_zones[i]=1
        print(conv_zones)
