...
T=np.ones(60)*2000
T=np.array([1953.37833987, 1953.44348421, 1953.57608236, 1953.75256365, 1954.019915, 1954.34494648, 1954.75512454, 1955.20406352, 1955.70008973, 1956.16457609, 1956.58003638, 1956.90818916, 1957.2228564,  1957.55232983, 1957.95230266, 1958.37144037, 1958.85433687, 1959.35821713, 1959.91416569, 1960.48791747, 1961.12035762, 1961.77532845, 1962.48551487, 1963.15944512, 1963.82577823, 1964.5148189,  1965.22988326, 1965.92436206, 1966.72941399, 1967.66965618, 1968.83234184, 1970.2504177,  1971.98909322, 1974.09489151, 1976.53255936, 1979.06697319, 1981.71931055, 1984.46020962, 1987.14186927, 1989.11681802, 1990.79451758, 1992.28279043, 1993.58553844, 1995.31140873, 1998.36432895, 2005.78482581, 2020.21136928, 2052.84854457, 2111.07964617, 2230.80696111, 2407.24919018, 2684.89833253, 3011.98431775, 3422.99331602, 3817.57742879, 4239.54274112, 4636.16310509, 5064.84614675, 5508.73189373, 6000.        ])
...
def take_step_tp_trad(TList,in_levels,N,extinctCoef,scatCoef,conv_lays,doplot=False,firstplot=False):
    ...
    conv=True
    if conv:
        convt= get_conv_flux(in_levels,extinctCoef)
    ...
    #qqq[50:]=1e3
    #pdb.set_trace()
    delta_levels_lin = np.linalg.lstsq(A, sigmaSB*500**4+deltafluxsum+convt, rcond=None)[0]
    ...
    new_conv_lays=find_conv_lays(new_levels)[0]   
    
    for i in new_conv_lays:
        atm.conv_lays[i]=1
    
    #conv= get_conv_flux(T,extinctCoef)
    #pdb.set_trace()
    
    for i in np.where(atm.conv_lays==1)[0]:
        new_levels[i+1]=np.exp(np.log(new_levels[i])+2./7.*np.log(atm.p[i+1]/atm.p[i]))
...
def find_conv_lays(T):
    ...
    #            grad_ad = 0.15
    grad_ad = 2./7. # Genesis
    convLay = np.where(grad >= grad_ad+1e-5)[0]
...
def get_conv_flux(T,chi_extinctCoef):
    ...
    grad = deepcopy(find_conv_lays(T)[1])
