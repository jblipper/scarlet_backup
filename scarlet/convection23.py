...
def tp_trad(T,plot=True):
    ...
    conv_flux=np.zeros(N)
    while np.max(np.abs(deltas))>0.5:
        deltas,levels=take_step_tp_trad(TList,levels,N,extinctCoef,scatCoef,conv_zones,conv_flux,doplot,firstplot)
    ...
    conv_flux=get_conv_flux(levels,extinctCoef)
    
    #Convection
    convection=True
    if convection==True:
        levels=conv_corr(atm,levels,conv_zones,TList,extinctCoef,scatCoef,conv_flux)    
        atm.T=levels
        plotTp(forceLabelAx=True)
        TList = np.c_[TList,levels]
        plotTpChanges(TList)      
    
def take_step_tp_trad(TList,in_levels,N,extinctCoef,scatCoef,conv_zones,conv_flux,doplot=False,firstplot=False):
    ...
    #pdb.set_trace()
    delta_levels_lin = np.linalg.lstsq(A, (sigmaSB*500**4+deltafluxsum+conv_flux*conv_zones), rcond=None)[0]
    delta_levels_lin = np.concatenate((delta_levels_lin, np.zeros(atm.nLay-N)))
    #pdb.set_trace()
...
def conv_corr(atm,T,conv_zones,TList,extinctCoef,scatCoef,conv_flux):
    plotTp(forceLabelAx=True)
    plotTpChanges(TList)
    print('convection')
    ...
    #            grad_ad = 0.15
    grad_ad = 2./7. # Genesis
    slopeTooSteep = np.where(grad >= grad_ad)[0]
    if len(slopeTooSteep>0):
        conv_lay = slopeTooSteep[0]
        print(f'conv_lay:{conv_lay}')
        T[conv_lay+1]=np.exp(np.log(T[conv_lay])+2./7.*np.log(atm.p[conv_lay+1]/atm.p[conv_lay]))
        #T[conv_lay]=5000
        #T[conv_lay+1]=3700
        ...
        while np.max(np.abs(deltas*(1-conv_zones)))>0.5:
            deltas,T=take_step_tp_trad(TList,T,atm.nLay-np.sum(conv_zones==1)+1,extinctCoef,scatCoef,conv_zones,conv_flux,doplot=False,firstplot=False)
            for zone in np.where(conv_zones == 1)[0]:
                #pdb.set_trace()
                T[zone+1]=np.exp(np.log(T[zone])+2./7.*np.log(atm.p[zone+1]/atm.p[zone]))
            TList = np.c_[TList,T]
            print(deltas)
        T=conv_corr(atm,T,conv_zones,TList,extinctCoef,scatCoef,conv_flux)       
...
def get_conv_flux(T,chi_extinctCoef):
    ...
    #Hconv_new=np.concatenate((np.ones(1)*Hconv[0],Hconv))
    #print(np.shape(Hconv))
