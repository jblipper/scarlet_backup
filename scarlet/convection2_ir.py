...
def take_step_tp_trad(TList,in_levels,N,extinctCoef,scatCoef,conv_zones,doplot=False,firstplot=False):
    ...
    plotTpChanges(TList)
    atm.T=in_levels
    toon= get_fluxes_toon(atm,atm.IrradStar,extinctCoef,scatCoef,ref=False)
    ...
    for level_i in range(N_rad):
        ...
        atm.T=levels_ptb
        flux_ptb_all=get_fluxes_toon(atm,atm.IrradStar,extinctCoef,scatCoef,ref=False)
    ...
    delta_levels_lin = np.linalg.lstsq(A, (sigmaSB*500**4+deltafluxsum), rcond=None)[0]
