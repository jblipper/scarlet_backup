...
def tp_trad(T,plot=True):
    ...
    while np.max(np.abs(deltas))>0.5:
        ...
        print(count)  
        print(levels)
    E_radiative=calcEnergy(levels)
    #Convection
    convection=True
    if convection==True:
        temp=deepcopy(levels)
        print(f'levels:{levels}')
        levels1=conv_corr(atm,levels,conv_zones,TList,extinctCoef,scatCoef,False)    
        TList = np.c_[TList,levels1]
        print(f'levels1:{levels1}')
        E_convective_up=calcEnergy(levels1)
        levels=deepcopy(temp)
        levels2=conv_corr(atm,levels,conv_zones,TList,extinctCoef,scatCoef,True) 
        TList = np.c_[TList,levels2]
        E_convective_down=calcEnergy(levels2)
        levels=temp
        print(E_convective_up)
        print(E_radiative)
        print(E_convective_down)
        #print(E_convective_down)
        atm.T=levels
...
def conv_corr(atm,T,conv_zones,TList,extinctCoef,scatCoef,from_bottom):
    plotTp(forceLabelAx=True)
    plotTpChanges(TList)
    print('convection')
    dT=np.diff(T)
    ...
    if from_bottom==True:
        slopeTooSteep = (np.where(dTdz > dTdzAdiabat+1e-8)[0])[::-1] 
    else:
        slopeTooSteep = (np.where(dTdz > dTdzAdiabat+1e-8)[0])
    ...
    if len(slopeTooSteep>0):
        ...
        if from_bottom==True:
            T[conv_lay]=T[conv_lay+1]-dTdzAdiabat[conv_lay]*atm.dz[conv_lay]
        else:
        ...
        #    print(deltas)
        #    print(np.max(np.abs(deltas*(1-conv_zones))))
        #    deltas,T=take_step_tp_trad(TList,T,atm.nLay,extinctCoef,scatCoef,conv_zones,doplot=False,firstplot=False)
        #    
        #    temp=deepcopy(atm.T)
        #    atm.T=deepcopy(T)   
        #    toonc= get_fluxes_toon(atm,atm.IrradStar,extinctCoef,scatCoef)
        #    nets=np.trapz(x=atm.wave,y=toonc[0]-toonc[1])
        #    print('nets')
        #    print(nets)
        #    print('only_rad')
        #    print(conv_zones)
        #    #print(np.dot(nets*(1-conv_zones)))       
        #    atm.T=deepcopy(temp)
        #    
        TList = np.c_[TList,T]
        #    print(deltas)
        deltas=np.zeros(atm.nLay)
        T=conv_corr(atm,T,conv_zones,TList,extinctCoef,scatCoef,from_bottom) 
