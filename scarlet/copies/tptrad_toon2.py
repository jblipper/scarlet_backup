...
#    flux00=get_flux(T,flux_star)#+toon[2]
    ...
    def fun(t, temps):
        atm.T=temps
        flux0_full=atm.multiScatToon(atm.IrradStar,extinctCoef,scatCoef,refl = True,thermal = True)[2] 
        flux0=np.trapz(x=atm.wave,y=flux0_full)
        net_fluxes=np.zeros(N)
        for i in range(1,N-1):
            net_fluxes[i]=flux0[i]-flux0[i-1]       
        rho = atm.ntot*atm.MuAve
        massLay = rho * atm.z
        heatCap = 20.79/1.01e-3 * massLay 
        heatCap[-1]=7*10**11  
        dTdt=net_fluxes/heatCap*1000
        print('i')
        return dTdt 
    ...
    t_b=100
    tt=(0,100)
    t_eval = np.linspace(tt[0], tt[1], 100) 
    flux0_full=toon[2] 
    y0=levels
    pdb.set_trace() 
    solution=solve_ivp(fun, tt, y0, t_eval=t_eval)
    print('**')
    pdb.set_trace() 
