...
def get_fluxes_toon(atm,IrradStar,extinctCoef,scatCoef,temperature_profile,asym = 0.725,u0 = 0.5773502691896258, intTopLay=0,method_for_therm ='Hemispheric Mean', method_for_ref='Quadrature', hard_surface=False, surf_reflect=0, mid=False, ftau_cld=0, b_top_for_ref=0, ref=True, therm=True, w0_f=0):
    ...
    if ref==True:
        ...
        if delta_approx == True :
            dtau_og=deepcopy(dtau)
            dtau=dtau*(1.-w0*asym**2)
            tau_og=deepcopy(tau)
            tau[0]=tau[0]*(1.-w0[0]*asym[0]**2)
            for i in range(nlay):
                tau[i+1]=tau[i]+dtau[i]
        
        ##### --SM-- need to correct the tau arrays first and the w0 and asym arrays later
            w0_og=deepcopy(w0)
            w0=w0*((1.-asym**2)/(1.-w0*(asym**2)))
            asym_og=deepcopy(asym)
            asym=asym/(1.+asym)
        ...
        xint_at_top=np.zeros(atm.nWave)
        
        gaussweights=np.array([0.015747,0.073908,0.146386,0.167174,0.096781])
        ulist=np.array([0.99513358,0.95250091,0.82712012,0.59734205,0.27934726])
        
        #gaussweights=np.array([0.5])
        #ulist=np.array([0.5773502691896258])
        ...
        for g in range(len(ulist)):
            ...
            if method_for_ref == 'Eddington': #Eddington Method
                gam1  = (7-w0*(4+3*ftau_cld*asym))/4 #(sq3*0.5)*(2. - w0*(1.+asym)) #Table 1 Toon et Al. 
                gam2  = -(1-w0*(4-3*ftau_cld*asym))/4 #(sq3*w0*0.5)*(1.-asym)       #Table 1 Toon et Al.
            ...
            if do_xint==True:
                ...
                #use expression for bottom flux to get the flux_plus and flux_minus at last
                #bottom layer
                flux_zero  = positive[-1,:]*exptrm_positive[-1,:] + Gam[-1,:]*negative[-1,:]*exptrm_minus[-1,:] + C_plus_down[-1,:]


                xint = zeros((nlev,nWave))
                xint[-1,:] = flux_zero/pi
                ...
                if multi_phase ==0:#'N=2':
                    ...
                    ubar2 = 0.767  # 
                    gcos2=0.5
                    multi_plus = (1.0+1.5*ftau_cld*asym*u1 #!was 3
                                    + gcos2*(3.0*ubar2*ubar2*u1*u1 - 1.0)/2.0)
                    multi_minus = (1.-1.5*ftau_cld*asym*u1 
                                    + gcos2*(3.0*ubar2*ubar2*u1*u1 - 1.0)/2.0)
                elif multi_phase ==1:#'N=1':
                    multi_plus = 1.0+1.5*ftau_cld*asym*u1  
                    multi_minus = 1.-1.5*ftau_cld*asym*u1
                ################################ end options for multiple scatteirng ####################

                G=positive*(multi_plus+Gam*multi_minus)    *w0
                H=negative*(Gam*multi_plus+multi_minus)    *w0
                ...
                if single_phase!=1: 
                    g_forward = constant_forward*asym_og
                    g_back = constant_back*asym_og#-
                ...
                # p_single=(1-asym_og**2)/sqrt((1+asym_og**2+2*asym_og*cos_theta)**3) 
                # as opposed to the traditional:
                # p_single=(1-asym_og**2)/sqrt((1+asym_og**2-2*asym_og*cos_theta)**3) (NOTICE NEGATIVE)
                ...
                for i in range(nlay-1,-1,-1):
                    ...
                    xint[i,:] =( xint[i+1,:]*exp(-dtau[i,:]/u1) 
                            ...
                            *(1. - exp(-dtau_og[i,:]*(u0+u1)
                            ...
                            (u0/(u0+1*u1))
                            +G[i,:]*(exp(exptrm[i,:]*1-dtau[i,:]/u1) - 1.0)/(Lam[i,:]*1*u1 - 1.0)
                            +H[i,:]*(1. - exp(-exptrm[i,:]*1-dtau[i,:]/u1))/(Lam[i,:]*1*u1 + 1.0)
                            )


                xint_at_top += xint[0,:] * weight
...
#                xint[-1,:] = flux_zero/pi
        ...
        if mid==False:
            ...
            reflected_component = flux_plus_all,flux_minus_all,xint_at_top
        else:
            reflected_component = (flux_plus_midpt_all,flux_minus_midpt_all), (flux_plus_all, flux_minus_all)
...
w0_f=np.linspace(0.001,0.99999,100)
w0_f=np.linspace(0.7,0.99999,100)
#w0_f=np.array([0.999999])
geoms=[]
for i in w0_f:
    Fupw, Fdwn, xint=get_fluxes_toon(atm,atm.IrradStar,extinctCoef,scatCoef,atm.T,therm=False,ref=True,asym=0, w0_f=i)
    #atm.albedoToon = Fupw[0,:] / atm.IrradStar

    #total_toon_emis=np.trapz(Fupw[0],atm.wave)

    total_toon_emis=np.trapz(xint,atm.wave)
    
    total_irrad=np.trapz(atm.IrradStar,atm.wave)

    geometric_albedo=np.pi*total_toon_emis/total_irrad*2
    
    #geometric_albedo=np.mean(np.pi*xint/atm.IrradStar)
