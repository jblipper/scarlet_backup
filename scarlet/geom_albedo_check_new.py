...
w0_f=np.linspace(0.001,0.999,1)
geoms=[]
for i in w0_f:
    ...
    geometric_albedo=Fplanet/atm.fStarSurf*(atm.ap/atm.Rp)**2
    
    plt.plot(atm.wave,geometric_albedo)
    
    plt.xscale('log')
    
    plt.savefig('geom_albedo_wave.png')
    
    #total_toon_emis=np.trapz(Fupw[0],atm.wave)

    #total_irrad=np.trapz(atm.IrradStar,atm.wave)

    #geometric_albedo_integrated=total_toon_emis/total_irrad*(atm.ap/atm.Rp)**2
    
    geometric_albedo_integrated=np.trapz(geometric_albedo,atm.wave)
    
    print(geometric_albedo_integrated)
    
    geoms.append(geometric_albedo_integrated)
