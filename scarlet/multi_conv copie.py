...
def take_step_tp_trad(TList,in_levels,N,extinctCoef,scatCoef,conv_zones,doplot=False,firstplot=False,reduce_step=True):
...
def conv_corr(atm,T,conv_zones,TList,extinctCoef,scatCoef):    
    ...
    if len(slopeTooSteep>0):
        ...
        #for ube in range(1):
            deltas,T=take_step_tp_trad(TList,T,atm.nLay,extinctCoef,scatCoef,conv_zones,doplot=False,firstplot=False,reduce_step=False)
            for zone in np.where(conv_zones == 1)[0]:
                #pdb.set_trace()
