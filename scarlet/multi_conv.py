...
def tp_trad(T,plot=True):
    ...
    while np.max(np.abs(deltas))>0.5:
        deltas,levels=take_step_tp_trad(TList,levels,N,extinctCoef,scatCoef,is_conv,doplot,firstplot)
    ...
    convection=True
    if convection==True:
        levels=conv_corr(atm,levels,is_conv,TList,extinctCoef,scatCoef)    
        atm.T=levels
        plotTp(forceLabelAx=True)
        TList = np.c_[TList,levels]
        plotTpChanges(TList)      
    
def take_step_tp_trad(TList,in_levels,N,extinctCoef,scatCoef,is_conv,doplot=False,firstplot=False,reduce_step=True):
    is_rad=1-is_conv
    #pdb.set_trace()
    rad_zones=[]
    rad_zone=[]
    ...
    for level_i in range(N_rad):
        rad_lev_i=rad_levs[level_i]
        print('*****')
        print(rad_lev_i)
        print('*****')
        deltaT=0.001
        levels_ptb=in_levels.copy()
        levels_ptb[rad_lev_i]=in_levels[rad_lev_i]+deltaT
        if rad_lev_i<N-1:
            if is_rad[rad_lev_i+1]==0:
                print('zzz')
    ...
    delta_levels_lin=np.zeros(N)
    rad_num=0
    for i in range(N):
        if is_rad[i]==True:
            delta_levels_lin[i]=delta_levels_lin_rad[rad_num]
            rad_num+=1
        else:
            delta_levels_lin[i]=delta_levels_lin[i-1]
    ...
    if reduce_step==True:
        ...
        delta_levels_reduced=delta_levels_lin*1.0
    maxabs=np.max(np.abs(delta_levels_reduced))
    if maxabs>250:
        delta_levels_reduced=delta_levels_reduced*250/maxabs
    new_levels=in_levels+delta_levels_reduced#*is_rad
    
    #for zone in np.where(is_conv == 1)[0]:
    #    #pdb.set_trace()
...
def conv_corr(atm,T,is_conv,TList,extinctCoef,scatCoef):    
    ...
    if len(slopeTooSteep>0):
        ...
        is_conv[conv_lay+1:conv_lay+2]=1
        deltas=np.ones(atm.nLay)*11
        print(f'conv:{is_conv}')
        while np.max(np.abs(deltas*(1-is_conv)))>0.5:
        #for ube in range(1):
            deltas,T=take_step_tp_trad(TList,T,atm.nLay,extinctCoef,scatCoef,is_conv,doplot=False,firstplot=False,reduce_step=False)
            for zone in np.where(is_conv == 1)[0]:
                #pdb.set_trace()
                T[zone]=T[zone-1]+dTdzAdiabat[zone-1]*dz[zone-1]
            TList = np.c_[TList,T]
            print(deltas)
        T=conv_corr(atm,T,is_conv,TList,extinctCoef,scatCoef)       
