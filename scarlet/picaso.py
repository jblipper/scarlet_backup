...
def get_flux_toon(nlevel, wno, nwno, tau, dtau, w0, cosbar, surf_reflect, ubar0, F0PI):
    ...
    if tridiagonal==0:
        A, B, C, D = setup_tri_diag(nlayer,nwno,  c_plus_up, c_minus_up, c_plus_down, c_minus_down, b_top, b_surface, surf_reflect, gama, dtau, exptrm_positive,  exptrm_minus) 
    else:
        A_, B_, C_, D_, E_, F_ = setup_pent_diag(nlayer,nwno,  c_plus_up, c_minus_up, c_plus_down, c_minus_down, b_top, b_surface, surf_reflect, gama, dtau, exptrm_positive,  exptrm_minus, g1,g2,exptrm,lamda) 
...
atm.dz=np.append(atm.dz,np.array([atm.dz[-1]]))  
...
cosbar=0
surf_reflect=0
ubar0=0.5773502691896258
F0PI=atm.IrradStar

tridiagonal=0

pdb.set_trace()
flux_up, flux_down = get_flux_toon(nlevel, wno, nwno, tau, dtau, w0, cosbar, surf_reflect, ubar0, F0PI)
...
plt.ylim(-1000,1800000)
#plt.plot(atm.wave[7900:],toon[0][0][7900:])
plt.savefig('picasso_up.png')
plt.clf()
print(flux_up[0])
