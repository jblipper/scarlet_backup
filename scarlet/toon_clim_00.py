...
def get_fluxes( pressure, temperature, dwni, DTAU, TAU, W0, COSB,ftau_cld, surf_reflect, ubar0,ubar1, F0PI,
    ...
    if reflected:
        ...
        for ig in range(ngauss): # correlated - loop (which is different from gauss-tchevychev angle)
            ...
            toaintensity,out_ref_fluxes = get_reflected_1d(nlevel, wno,nwno,ng_clima,nt_clima,
                                    DTAU[:,:], TAU[:,:], W0[:,:], COSB,
                                    ftau_cld,surf_reflect, ubar0_clima,ubar1_clima,
                                    F0PI, get_toa_intensity=1, get_lvl_flux=0)
                                    
            pdb.set_trace()
            
            plt.plot(atm.wave,toaintensity[0][0]*pi)
            plt.title('Downwards Layer 30')
            plt.xscale('log')
            plt.xlabel('Wavelength')
            plt.ylabel('Intensity')
            #plt.ylim(-1,500000)
            plt.savefig('toon_clim/toaintensity.png')
            plt.clf()
