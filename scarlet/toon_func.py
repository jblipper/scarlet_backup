...
def multiScatToon(IrradStar,extinctCoef,scatCoef,asym = 0,u0 = 0.5773502691896258,refl = True,thermal = True,calcJ = False,intTopLay=0,method ='Hemispheric Mean',w = -1):
    '''
...
#        Fupw = FP1 + FP2 + CP(tauc[1,:],tautest,1)
    ...
    if thermal:    
        ...
        #atm.g1 = 2.0 - atm.omega*(1+0); atm.g2 = atm.omega*(1-0) #added

        ##alpha = sqrt( (1.-w0) / (1.-w0*cosb) )    #added
        #atm.lmbda = np.sqrt(atm.g1**2 - atm.g2**2) #eqn 21 toon   #added
        #atm.gamma = (atm.g1-atm.lmbda)/atm.g2 # #eqn 22 toon          #added
    
        ##g1_plus_g2 = 1.0/(g1+g2) #second half of eqn.27   #added
...
toon= multiScatToon(atm.IrradStar,extinctCoef,scatCoef,refl =False,thermal = True, calcJ = False, intTopLay=0, w=-1)

#for qw in range(59):
#    toon[0][qw][:7900]=0
#    toon[1][qw][:7900]=0

plt.plot(atm.wave,toon[1][30])
plt.title('Downwards Flux Layer 30')
#plt.xscale('log')
plt.xlabel('Wavelength')
plt.ylabel('Intensity')
plt.ylim(-1000,1000)
#plt.plot(atm.wave[7900:],toon[0][0][7900:])
plt.savefig('toonfunct2.png')
plt.clf()
print(toon[0][0])
