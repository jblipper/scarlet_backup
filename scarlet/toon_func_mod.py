...
def multiScatToon(IrradStar,extinctCoef,scatCoef,asym = 0,u0 = 0.5773502691896258,refl = False,thermal = True,calcJ = False,intTopLay=0,method ='Hemispheric Mean',w = -1):
    ...
    for j in range(1,l):
        ...
        E[k] = e3[j,:]*(atm.CP(tauc[j+1,:],0,j+1) - atm.CP(tauc[j,:],taun[j,:],j)) - e1[j,:]*(atm.CM(tauc[j+1,:],0,j+1)-atm.CM(tauc[j,:],taun[j,:],j)) 
        #print j,k
        # check indicies
    ...
    for j in range(1,l):
        ...
        E[k] = e2[j+1,:]*(atm.CP(tauc[j+1,:],0,j+1) - atm.CP(tauc[j,:],taun[j,:],j)) - e4[j+1,:]*(atm.CM(tauc[j+1,:],0,j+1)-atm.CM(tauc[j,:],taun[j,:],j))
        #print j,k
        # check indicies
    pdb.set_trace()
    ...
    D[k] = 0
    E[k] = GroundAlbedo*atm.u0*np.exp(-tauc[l,:]/atm.u0)*np.pi*atm.Fs - atm.CP(tauc[l,:],taun[l,:],l) + GroundAlbedo*atm.CM(tauc[l,:],taun[l,:],l)
...
#        Fupw = FP1 + FP2 + atm.CP(tauc[1,:],tautest,1)
        for i in range(1,l+1):
            k = 2*i-1
            FP1 = solution[k,:]*(np.exp(-atm.lmbda[i,:]*(taun[i,:])) + atm.gamma[i,:]*np.exp(-atm.lmbda[i,:]*taun[i,:]))
            FP2 = solution[k+1,:]*(np.exp(-atm.lmbda[i,:]*(taun[i,:])) - atm.gamma[i,:]*np.exp(-atm.lmbda[i,:]*taun[i,:]))
            #pdb.set_trace()
            FupwSOL[i-1,:] = FP1 + FP2 + atm.CP(tauc[i,:],taun[i,:],i)
        ...
        for i in range(1,l+1):
            k = 2*i-1
            FM1 = solution[k,:]*(atm.gamma[i,:]*np.exp(-atm.lmbda[i,:]*(taun[i,:])) + np.exp(-atm.lmbda[i,:]*taun[i,:]))
            FM2 = solution[k+1,:]*(atm.gamma[i,:]*np.exp(-atm.lmbda[i,:]*(taun[i,:])) - np.exp(-atm.lmbda[i,:]*taun[i,:]))
            DirSOL[i-1,:] = atm.u0*np.pi*atm.Fs*np.exp(-(tauc[i,:]+taun[i,:])/atm.u0)
            FdwnSOL[i-1,:] = FM1 + FM2 + atm.CM(tauc[i,:],taun[i,:],i) + DirSOL[i-1,:]
                
        # Net Flux
        FnetSOL = np.zeros([l,d], dtype=atm.numerical_precision)
        for i in range(1,l+1):
            k = 2*i-1               
            FnetSOL[i-1,:] = solution[k,:]*(e1[i,:]-e3[i,:])+solution[k+1,:]*(e2[i,:]-e4[i,:])+ atm.CP(tauc[i],taun[i],i) - atm.CM(tauc[i],taun[i],i)        
...
#             plt.clf()
    ...
    if calcJ:
        ...
        for i in range(1,l+1):
            k = 2*i-1      
            DirSOL[i-1,:] = atm.u0*np.pi*atm.Fs*np.exp(-(tauc[i,:]+taun[i,:])/atm.u0)
            JSOL[i-1,:] = (solution[k,:]*(e1[i,:]+e3[i,:])+solution[k+1,:]*(e2[i,:]+e4[i,:])+ atm.CP(tauc[i],taun[i],i) - atm.CM(tauc[i],taun[i],i) + DirSOL[i-1,:]/u0)/(4*np.pi*u1)   
