...
def get_fluxes_toon(atm,IrradStar,extinctCoef,scatCoef,asym = 0,u0 = 0.5773502691896258, intTopLay=0,method_for_therm ='Hemispheric Mean', method_for_ref='Quadrature', hard_surface=False, surf_reflect=0, mid=False, ftau_cld=0, b_top_for_ref=0, ref=True, therm=True):
...
def tri_diag_solve(l, a, b, c, d):
    ...
    return XK    
flux_up,flux_down=get_fluxes_toon(atm,atm.IrradStar,extinctCoef,scatCoef,therm=True,ref=False)
pdb.set_trace()
...
#plt.ylim(-1,5000)
plt.savefig('toonew/flux_up_top.png')
plt.clf()
...
plt.ylabel('Flux (W/(m^2*um))')
#plt.ylim(-1,5000000)
plt.savefig('toonew/flux_down_top.png')
plt.clf()
