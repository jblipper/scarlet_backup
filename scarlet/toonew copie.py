...
def get_thermal(atm,IrradStar,extinctCoef,scatCoef,asym = 0,u0 = 0.5773502691896258, intTopLay=0,method ='Hemispheric Mean', hard_surface=False, surf_reflect=0):
...
def get_reflected():
    ...
    #C_plus=w0*np.pi*IrradStar*np.exp(-(tau)/u0)*((gam1-1/u0)*gam3+gam4*gam2)/denominator #Equation 23 Toon et Al.
    
    #C_minus=w0*np.pi*IrradStar*np.exp(-(tau)/u0)*((gam1+1/u0)*gam4+gam2*gam3)/denominator #Equation 24 Toon et Al.
...
def tri_diag_solve(l, a, b, c, d):
    ...
    return XK    
get_thermal(atm,atm.IrradStar,extinctCoef,scatCoef)
