...
def get_fluxes_toon(atm,IrradStar,extinctCoef,scatCoef,asym = 0,u0 = 0.5773502691896258, intTopLay=0,method_for_therm ='Hemispheric Mean', method_for_ref='Quadrature', hard_surface=False, surf_reflect=0, only_mid=True, ftau_cld=0, b_top_for_ref=0, ref=True, therm=True):
    ...
    if therm==True:
        ...
        C_plus_down = 2*pi*u1*(b0 + b1 * dtau + b1 * gamterm) #2 removed
        C_minus_down = 2*pi*u1*(b0 + b1 * dtau - b1 * gamterm) #2 removed
        ...
        K = (1/u1 - Lam)*negative     
        alpha1 = 2*pi*(b0+b1*(gamterm - u1)) #2 removed
        alpha2 = 2*pi*b1 #2 removed
        sigma1 = 2*pi*(b0-b1*(gamterm - u1)) #2 removed
        sigma2 = 2*pi*b1 #2 removed
        ...
        if hard_surface:
            flux_plus[-1,:] = bb_all[-1,:] *2*pi +sigmaSB*100**4  # terrestrial flux /pi = intensity #2 removed
        else:
            flux_plus[-1,:] = ( bb_all[-1,:] + b1[-1,:] * iubar)*2*pi +sigmaSB*100**4 #no hard surface   #2 removed 
        ...
        if only_mid==True:
            thermal_component = flux_plus_mdpt*0.5,flux_minus_mdpt*0.5 #0.5 from sum of gauss weights
        else:
            thermal_component = (flux_plus_mdpt*0.5,flux_minus_mdpt*0.5), (flux_plus*0.5, flux_minus*0.5)
        
    if ref==True:
    
        if method_for_ref == 'Eddington': #eddington
        ...
        if only_mid==True:
            reflected_component = flux_plus_midpt,flux_minus_midpt
        else:
