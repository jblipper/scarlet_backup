...
def take_step_tp_trad(TList,in_levels,N,extinctCoef,scatCoef,doplot=False,firstplot=False):
    ...
    plt.clf()
    plt.imshow(A, cmap='viridis', norm=SymLogNorm(linthresh=1, linscale=0.1, base=10), interpolation='nearest',label=('dF(A)/dT(B)'))
    plt.xlabel('Level X')
    plt.ylabel('Layer Y')
    cbar=plt.colorbar()  # Add a colorbar to the heatmap
    cbar.set_label('dF(Y)/dT(X)')
