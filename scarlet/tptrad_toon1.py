...
def tp_trad(T,plot=True):
    ...
    doplot=True
    firstplot=True
    while np.max(np.abs(deltas))>1:
        deltas,levels=take_step(levels,N,extinctCoef,scatCoef,doplot,firstplot)
        firstplot=False
        #doplot=False
        count+=1
        print(count)
...
def take_step(in_levels,N,extinctCoef,scatCoef,doplot=False,firstplot=False):
    atm.T=in_levels
    ...
    for level_i in range(N):
        ...
        atm.T=levels_ptb
        flux_ptb_all=atm.multiScatToon(atm.IrradStar,extinctCoef,scatCoef,refl = True,thermal = True)
