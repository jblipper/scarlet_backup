...
def take_step(TList,in_levels,N,extinctCoef,scatCoef,doplot=False,firstplot=False):
    plotTp(forceLabelAx=True)
    plotTpChanges(TList)
    atm.T=in_levels
    toon= atm.multiScatToon(atm.IrradStar,extinctCoef,scatCoef,refl = True,thermal = True,w=-1)
    ...
    for level_i in range(N):
        ...
        atm.T=levels_ptb
        flux_ptb_all=atm.multiScatToon(atm.IrradStar,extinctCoef,scatCoef,refl = True,thermal = True,w=-1)
